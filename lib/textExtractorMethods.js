...
const LANGUAGE_FILE_PATH = config.BASE_CODE_PATH;
...
`;
  ...
  if (reactLabelMatches !== null) {
    let ignoreFields = {
      alignment:                      true,
      autoCapitalize:                 true,
      backButtonOverrideViewNameOrId: true,
    ...
    for ( let i = 0; i < reactLabelMatches.length; i++) {
      ...
      while ((resultArray = reactLabelRegex.exec(match)) !== null) {
        if (ignoreFields[resultArray[1]] === undefined) {
          ...
          let f5 = resultArray[2].substr(1,5);
          if (ignorePrefixes3[f3] === undefined && ignorePrefixes4[f4] === undefined && ignorePrefixes5[f5] === undefined) {
            if (resultArray[0].indexOf("lang(\"") == -1) {
              console.log("Using the value of the following key: ", resultArray[1], resultArray[2])
...
function createTranslationFileAndReplaceContents(filename, filePath, extractData, target, targetType, contentData, openWithCurly = false, paddingArray = null) {
  ...
  target[filename][textKey] = wrap(extractData.text);
...
module.exports = {parseFile, parseFilesRecursivelyInPath, clearTranslationData, wrap}
