...
let allowReplace = util.isArgvTrue('replace') ?? false;
  ...
  if (translationKeys.length > 0) {
    if ( __dataBlob[filename] !== undefined) {
      ...
      if (!conflict) {
        ...
        extractionMethods.parseFile(fileMap[filename], allowReplace)
        translationKeys.forEach((translationKey) => {
...
console.log("Finished!")
  ...
  console.log("This was a dry run. No files were written to disk. To write to disk, set allowReplace to true.");
}
